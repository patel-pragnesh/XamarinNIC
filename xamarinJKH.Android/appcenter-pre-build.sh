...
if [ ${#LABEL} -gt 0 ]
 then
    if [ -a ${ACTIVITY} ]; then
        sed "s/Label = \"[-a-zA-Z0-9 | _ | ' ' | А-Яа-я]*\"/Label = \"${LABEL}\"/" -i ${ACTIVITY} 
...
if [ ${#PACKAGENAME} -gt 0 ]
 ...
 then
    ...
    sed "s/android:label=\"[-a-zA-Z0-9а-яА-Я | ' ']*\"/android:label=\"${LABEL}\"/" -i ${MANIFEST}
    sed "s/package=\"[a-z0-9 | . | _]*\"/package=\"${PACKAGENAME}\"/" -i ${MANIFEST}
 fi
